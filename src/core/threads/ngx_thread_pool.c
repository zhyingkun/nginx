...
static ngx_int_t ngx_thread_pool_init(ngx_thread_pool_t* tp, ngx_log_t* log, ngx_pool_t* pool) {
  ...
  if (err) {
    ngx_log_error(NGX_LOG_ALERT, log, err, "pthread_attr_setstacksize() failed");
    return NGX_ERROR;
  }
#endif
...
ngx_int_t ngx_thread_task_post(ngx_thread_pool_t* tp, ngx_thread_task_t* task) {
  ...
  if (tp->waiting >= tp->max_queue) {
    ...
    ngx_log_error(
        NGX_LOG_ERR, tp->log, 0, "thread pool \"%V\" queue overflow: %i tasks waiting", &tp->name, tp->waiting);
    return NGX_ERROR;
  }
