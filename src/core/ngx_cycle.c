...
old_shm_zone_done:
  ...
  for (i = 0; i < old_cycle->listening.nelts; i++) {
    ...
    if (ngx_close_socket(ls[i].fd) == -1) {
      ngx_log_error(
          NGX_LOG_EMERG, log, ngx_socket_errno, ngx_close_socket_n " listening socket on %V failed", &ls[i].addr_text);
    }
...
void ngx_reopen_files(ngx_cycle_t* cycle, ngx_uid_t user) {
  ...
  for (i = 0; /* void */; i++) {
    ...
    ngx_log_debug3(
        NGX_LOG_DEBUG_EVENT, cycle->log, 0, "reopen file \"%s\", old:%d new:%d", file[i].name.data, file[i].fd, fd);
...
static void ngx_shutdown_timer_handler(ngx_event_t* ev) {
  ...
  for (i = 0; i < cycle->connection_n; i++) {
    ...
    if (c[i].fd == (ngx_socket_t)-1 || c[i].read == NULL || c[i].read->accept || c[i].read->channel ||
        c[i].read->resolver) {
      continue;
    }
