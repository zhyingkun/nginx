...
static void ngx_show_version_info(void) {
  ...
  if (ngx_show_help) {
    ngx_write_stderr("Usage: nginx [-?hvVtTq] [-s signal] [-c filename] "
                     "[-p prefix] [-g directives]" NGX_LINEFEED NGX_LINEFEED "Options:" NGX_LINEFEED
                     "  -?,-h         : this help" NGX_LINEFEED "  -v            : show version and exit" NGX_LINEFEED
                     ...
                     "during configuration testing" NGX_LINEFEED "  -s signal     : send signal to a master process: "
...
static ngx_int_t ngx_add_inherited_sockets(ngx_cycle_t* cycle) {
    ...
    if (*p == ':' || *p == ';') {
      ...
      if (s == NGX_ERROR) {
        ngx_log_error(NGX_LOG_EMERG,
                      cycle->log,
                      0,
                      "invalid socket number \"%s\" in " NGINX_VAR " environment variable, ignoring the rest"
                      " of the variable",
                      v);
        break;
      }
  ...
  if (v != p) {
    ngx_log_error(NGX_LOG_EMERG,
                  cycle->log,
                  0,
                  "invalid socket number \"%s\" in " NGINX_VAR " environment variable, ignoring",
                  v);
  }
...
static char* ngx_core_module_init_conf(ngx_cycle_t* cycle, void* conf) {
...

  {
    ...
    if (lock_file.len) {
      ...
      ngx_memcpy(
          ngx_cpymem(cycle->lock_file.data, ccf->lock_file.data, ccf->lock_file.len), ".accept", sizeof(".accept"));
    }
  }
...
static char* ngx_load_module(ngx_conf_t* cf, ngx_command_t* cmd, void* conf) {
  ...
  if (modules == NULL) {
    ngx_conf_log_error(
        NGX_LOG_EMERG, cf, 0, ngx_dlsym_n " \"%V\", \"%s\" failed (%s)", &value[1], "ngx_modules", ngx_dlerror());
    return NGX_CONF_ERROR;
  }
  ...
  if (names == NULL) {
    ngx_conf_log_error(
        NGX_LOG_EMERG, cf, 0, ngx_dlsym_n " \"%V\", \"%s\" failed (%s)", &value[1], "ngx_module_names", ngx_dlerror());
    return NGX_CONF_ERROR;
  }
