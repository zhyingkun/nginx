...
char* ngx_log_set_log(ngx_conf_t* cf, ngx_log_t** head) {
  ...
  if (ngx_strcmp(value[1].data, "stderr") == 0) {
    ...
    needed = sizeof("MEMLOG  :" NGX_LINEFEED) + cf->conf_file->file.name.len + NGX_SIZE_T_LEN + NGX_INT_T_LEN +
             NGX_MAX_ERROR_STR;
    ...
    buf->pos =
        ngx_slprintf(buf->start, buf->end, "MEMLOG %uz %V:%ui%N", size, &cf->conf_file->file.name, cf->conf_file->line);
